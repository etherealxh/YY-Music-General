<template>
    <div class="yy-header">
        ...
        <div class="header-logo" @click="goPage()">
            <y-y-icon :icon="iconList.ERJI"></y-y-icon>
            <span>{{ musicName }}</span>
        </div>
        <y-y-header-nav class="yy-header-nav" :styleList="headerNavList" :activeName="activeNavName" @click="goPage"></y-y-header-nav>
        <!--搜索框-->
        <div class="header-search">
            <el-input placeholder="搜索" :prefix-icon="Search" v-model="keywords" @keyup.enter="goSearch()" />
        </div>
        <!--设置-->
        <y-y-header-nav v-if="!token" :styleList="signList" :activeName="activeNavName" @click="goPage"></y-y-header-nav>
...
<script lang="ts">
    ...
    import YYHeaderNav from "./YYHeaderNav.vue";
